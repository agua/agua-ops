...
has 'opsinfo'	=> ( isa => 'Ops::Info', is => 'rw', required	=>	0	);
...
method setUpInstall {
	...
	$self->setDbObject() if not defined $self->table()->db() or not defined $self->table()->db()->dbh();
...
method installDependencies ($opsdir, $installdir) {
	...
	foreach my $missed ( @$missing ) {
		...
		$self->logDebug("DOING object = Ops->new()");
		my $object = Ops->new({
			conf        =>  $self->conf(),
		...
		my $installedversion = $object->install();
...
method runInstall ($opsdir, $installdir, $package, $version) {
	...
	#### SET DATABASE OBJECT
	$self->setDbObject() if not defined $self->table()->db();
...
method zipInstallFiles ($installdir, $version) {
	...
	$self->runCommand("rm -fr $targetdir");
	...
	#### MAKE INSTALL DIRECTORY
	my $finaldir = "$installdir/$version";
	`rm -fr $finaldir/*` if -d $finaldir;
	$self->makeDir($finaldir) if not -d $finaldir;
    $self->changeDir($finaldir);

	#$self->logger()->write("Downloading file: $filename");
	$self->runCommand("wget -c $fileurl --output-document=$filename --no-check-certificate");
	
	my $filepath	=	"$installdir/$version/$filename";
	...
	#### CLEAN UP
	$self->runCommand("rm -fr $installdir/$version/$filename");
...
method zipInstall ($installdir, $version) {
	...
	if ( not -f $filepath or -z $filepath ) {
...
method setLogFile ($package, $random) {
	my $htmldir = $self->conf()->getKey("core:HTMLDIR");
...
method setHtmlLogFile {
	...
	my $htmldir		=	$self->conf()->getKey("core:HTMLDIR");
	my $urlprefix	=	$self->conf()->getKey("core:URLPREFIX");
